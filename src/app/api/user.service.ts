...
export class UserService {
  ...
  getRepresentations(): Observable<any> {
    return this.http.get(GlobalConstants.apiURL + '/representation/list', {
  ...
  updateFavorite(id: string) {
    return this.http.put(
      GlobalConstants.apiURL + `/auction/${id}/favorite`,
      null,
      { headers: { Authorization: this.getToken() } }
    );
  }
  ...
  saveRepresentation(userRepresentation: any): Observable<any> {
    ...
    return this.http
      .request('POST', GlobalConstants.apiURL + '/representation', {
        headers,
  ...
  updateUserNotifications(user: any) {
    return this.http.put(GlobalConstants.apiURL + '/user/notifications', user, {
      headers: { Authorization: this.getToken() },
    });
  }
  ...
  updateUserPassword(user: any): Observable<any> {
    return this.http.put(
      GlobalConstants.apiURL + '/user/change-password',
      user,
      { headers: { Authorization: this.getToken() } }
    );
  }
  ...
  updateUserData(user: any): Observable<any> {
    return this.http.put(GlobalConstants.apiURL + '/user', user, {
      headers: { Authorization: this.getToken() },
    });
  }
  ...
  userReestablecer(user: any): Observable<any> {
    return this.http.post(
      GlobalConstants.apiURL + '/auth/recover-password',
      user
    );
  }
  ...
  login(user: any): Observable<any> {
    return this.http.post(GlobalConstants.apiURL + '/auth/login', user).pipe(
      catchError((error) => {
        //return throwError(err);
        console.log(error);
        return throwError(() => error);
  ...
  verifyAccount(token: string): Observable<any> {
    return this.http
      .get(GlobalConstants.apiURL + '/auth/activate/' + token)
      .pipe(
        catchError((error) => {
          //return throwError(err);
          console.log(error);
  ...
  public bid(values: any, guid: string) {
    ...
    return forkJoin(
      paths.map(
        (path: string) =>
          this.http.post(GlobalConstants.apiURL + path, values, {
            headers: { Authorization: this.getToken() },
          }),
  ...
  public directSale(values: any, guid: string) {
    const paths: string[] = [`/direct_sale/${guid}`];
    return forkJoin(
      paths.map(
        (path: string) =>
          this.http.post(GlobalConstants.apiURL + path, values, {
